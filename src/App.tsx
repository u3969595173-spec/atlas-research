...
function App() {
  ...
  return <main className="app-shell">
    ...
    {showReview && <section className="review-panel"><div><p className="eyebrow">MIS SELECCIONES · {selected.length}/3</p><h2>Revisión antes de decidir</h2><p>Estas selecciones no son apuestas ni implican una recomendación.</p><ul>{reviewedMatches.map((match) => <li key={match.id}><b>{match.players}</b><span>{match.event} · Cuota {match.odds.toFixed(2)} · {label[match.classification]}</span></li>)}</ul></div><button className="icon-button" aria-label="Cerrar revisión" onClick={() => setShowReview(false)}><X size={18} /></button></section>}
    <section className="content-grid">
      ...
      <section className="analysis-panel"><div className="analysis-topline"><span>{active.event} · Hoy {active.time}</span><span className={`classification ${active.classification.toLowerCase()}`}>{label[active.classification]}</span></div><div className="title-row"><div><h2>{active.players}</h2><p>Mercado: ganador del partido</p></div><button className={`selection-button ${selected.includes(active.id) ? 'is-selected' : ''}`} onClick={() => toggleSelection(active.id)} disabled={analysisClosed || (!selected.includes(active.id) && selected.length >= 3)}>{selected.includes(active.id) ? <><Check size={16} /> En mis selecciones</> : <><Plus size={16} /> Seleccionar</>}</button></div>
        ...
        <section className="anomaly"><AlertTriangle size={19} /><div><b>ANOMALÍA — REQUIERE INVESTIGACIÓN</b><p>Diferencia de 0.06 entre la mejor y la peor cuota disponible. Por sí sola no indica un resultado ni una irregularidad.</p></div></section><section className="conclusion"><ClipboardCheck size={21} /><div><span>EXPLICACIÓN FINAL</span><b>REVISAR ANTES DE SELECCIONAR</b><p>La señal de ranking merece investigación; faltan fuentes fiables sobre forma, descanso y noticias recientes. Con la evidencia actual: NO BET.</p></div></section>
      </section>
      <aside className="control-panel"><div className="control-title"><ShieldCheck size={20} /><div><h2>Control personal</h2><p>Reglas activas para hoy</p></div></div><div className="selection-count"><span>SELECCIONES</span><b>{selected.length}<small> / 3 máximo</small></b><div className="progress"><i style={{ width: `${(selected.length / 3) * 100}%` }} /></div></div><ul className="rule-list"><li><Check size={16} /> Sin apuestas en vivo</li><li><Check size={16} /> Sin combinadas</li><li><Check size={16} /> Stake no aumenta tras pérdida</li><li><Check size={16} /> Máximo tres selecciones</li><li className={analysisClosed ? 'locked-rule' : ''}>{analysisClosed ? <LockKeyhole size={16} /> : <Check size={16} />} {analysisClosed ? 'Lista bloqueada' : 'Lista aún editable'}</li></ul><div className="no-bet-box"><b>NO BET es un resultado válido</b><p>Si ningún partido reúne evidencia suficiente, no se habilita ninguna selección.</p></div><button className="review-button" onClick={() => setShowReview(true)} disabled={selected.length === 0}>Revisar mis {selected.length} selecciones</button></aside>
