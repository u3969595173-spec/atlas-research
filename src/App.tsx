...
function App() {
  ...
  const loadFootballFixtures = () => {
    if (!apiUrl) return
    setFixturesLoading(true); setFootballFixtures([])
    fetch(`${apiUrl}/api/football/fixtures?league=${footballLeague}&date=${footballDate}`)
  ...
  const selectFootballFixture = (fixtureId: string) => {
  ...
  return <main className="app-shell">
    ...
    {showAdd && <section className="add-panel"><div className="add-fields"><select value={newSport} onChange={(event) => { setNewSport(event.target.value); setFootballFixtures([]); setNewPlayers(''); setNewEvent(''); setNewOddsSportKey('') }} aria-label="Deporte"><option>FUTBOL</option><option>BALONCESTO</option><option>BEISBOL</option><option>VOLEIBOL</option></select>{newSport === 'FUTBOL' ? <><select value={footballLeague} onChange={(event) => setFootballLeague(event.target.value)} aria-label="Competición"><option value="premier_league">Premier League</option><option value="la_liga">LaLiga</option><option value="serie_a">Serie A</option><option value="bundesliga">Bundesliga</option><option value="ligue_1">Ligue 1</option></select><input type="date" value={footballDate} onChange={(event) => setFootballDate(event.target.value)} aria-label="Fecha" /><button className="add-submit" onClick={loadFootballFixtures} disabled={fixturesLoading}>{fixturesLoading ? 'Buscando...' : 'Ver partidos'}</button>{footballFixtures.length > 0 && <select defaultValue="" onChange={(event) => selectFootballFixture(event.target.value)} aria-label="Partido"><option value="" disabled>Elige un partido</option>{footballFixtures.map((fixture) => <option value={fixture.id} key={fixture.id}>{fixture.home} vs {fixture.away}</option>)}</select>}{footballFixtures.length === 0 && !fixturesLoading && <span>Elige liga y fecha para cargar partidos.</span>}</> : <><input value={newPlayers} onChange={(event) => setNewPlayers(event.target.value)} placeholder="Participantes o equipos (A vs B)" autoFocus /><input value={newEvent} onChange={(event) => setNewEvent(event.target.value)} placeholder="Torneo o competición" /><input value={newOddsSportKey} onChange={(event) => setNewOddsSportKey(event.target.value)} placeholder="Identificador The Odds API" /></>}<button className="add-submit" onClick={addMatch} disabled={!newPlayers.trim() || !newEvent.trim() || !newOddsSportKey.trim()}>Añadir</button></div><button className="icon-button" aria-label="Cerrar" onClick={() => setShowAdd(false)}><X size={18} /></button></section>}
