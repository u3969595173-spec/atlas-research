...
function App() {
  ...
  const reopenAnalysis = () => {
    ...
    if (apiUrl) fetch(`${apiUrl}/api/workspaces/${workspaceId}/closed`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ closed: false }) }).then((response) => { if (!response.ok) throw new Error() }).catch(() => { setAnalysisClosed(true); setSyncError('No se pudo reabrir el análisis en la nube.') })
  }
  const addMatch = () => {
  ...
  const deleteAllMatches = () => {
    if (!window.confirm('¿Borrar todos los partidos y sus selecciones?')) return
    if (!apiUrl) {
      setStoredMatches([])
      setSelected([])
      setShowAdd(false)
    ...
    fetch(`${apiUrl}/api/workspaces/${workspaceId}/matches`, { method: 'DELETE' })
      .then(async (response) => { if (!response.ok) throw new Error(); return response.json() })
      .then(() => { setStoredMatches([]); setSelected([]); setShowAdd(false); setShowReview(false) })
      .catch(() => setSyncError('No se pudieron borrar los partidos.'))
  }
  ...
  return <main className="app-shell">
    ...
    <section className="workspace-header"><div><p className="eyebrow">LISTA PERSONAL · 21 SEPT 2026</p><h1>Partidos seleccionados</h1><p className="subhead">Analiza solo esta lista. La aplicación no busca eventos adicionales ni fuerza una selección.</p></div><div className="header-actions"><button className="secondary-button" onClick={() => setShowAdd(true)} disabled={analysisClosed}><Plus size={17} /> Añadir partido</button><button className="primary-button" onClick={analysisClosed ? reopenAnalysis : closeAnalysis}><LockKeyhole size={16} /> {analysisClosed ? 'Reabrir análisis' : 'Cerrar análisis'}</button></div></section>
    {syncError && <section className="cloud-error">{syncError}</section>}
    {analysisClosed && <section className="reopen-notice"><div><b>El análisis está cerrado</b><span>Reabre la lista para volver a añadir partidos y guardar candidatos.</span></div><button className="primary-button" onClick={reopenAnalysis}><RefreshCw size={16} /> Reabrir análisis</button></section>}
    {storedMatches.length > 0 && <section className="clear-list-panel"><span>Borrar la lista completa y sus selecciones.</span><button className="secondary-button" onClick={deleteAllMatches}><X size={16} /> Borrar todos los partidos</button></section>}
    ...
    {storedMatches.length ? <section className="content-grid">
    ...
    </section> : <section className="empty-state"><ClipboardCheck size={28} /><h2>Lista vacía</h2><p>Ya no hay partidos guardados. Puedes añadir uno nuevo cuando quieras.</p><button className="primary-button" onClick={() => setShowAdd(true)} disabled={analysisClosed}><Plus size={17} /> Añadir partido</button></section>}<footer>Atlas Research es una herramienta de análisis y control personal. No garantiza resultados ni sustituye una decisión responsable.</footer>
  </main>
}
export default App